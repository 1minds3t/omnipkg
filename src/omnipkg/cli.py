        ...
        if args.command == "config":
            ...
            if args.python_command == "adopt":
                ...
                if args.target.lower().startswith("python"):
                    ...
                    # ═══════════════════════════════════════════════════════════════
                    # BRANCH: Interactive vs CI/Non-interactive
                    # All shim setup, PATH manipulation, bat writing, and shell
                    # spawning is handled entirely by the dispatcher.
                    # cli.py only decides WHICH branch and calls into dispatcher.
                    # ═══════════════════════════════════════════════════════════════
                    if is_interactive:
                        ...
                        # Update the config (persistent)
                        result = pkg_instance.switch_active_python(version)
                        
                        if result == 0:
                            # Export to current process env (for this CI step)
