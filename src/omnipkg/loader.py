            ...
            for pkg_name in packages_to_cloak:
                ...
                for original_path in paths_to_find:
                    ...
                    if _ok:
                        ...
                        # Distinguish: file gone (race-safe) vs file still present (locked hostage)
                        if not original_path.exists():
                            # Already gone — another process beat us, that's fine.
                            if not self.quiet:
                                safe_print(
                                    f"      ℹ️  Already gone (race-safe skip): {original_path.name}"
                                )
                            successful_cloaks.append((original_path, cloak_path, False))
                        else:
                            # File is still there but we couldn't acquire the lock.
                            # Running the loader in this dirty state is worse than stopping.
                            raise RuntimeError(
                                f"HARD FAIL: Could not cloak '{original_path.name}' for package "
                                f"'{pkg_name}' — file is present but locked by another process. "
                                f"Cannot proceed with a dirty loader state. "
                                f"Resolve the lock conflict and retry."
                            )
    ...
    def _cleanup_all_cloaks_for_package(self, pkg_name):
        ...
        for cloak_info in cloaked_versions:
            ...
            if ok:
                ...
                # If the cloak file is gone, someone else won the race — skip.
                # If it's still there, we have a hostage situation: hard fail.
                if cloak_path.exists():
                    raise RuntimeError(
                        f"HARD FAIL: Could not restore cloaked file '{original_name}' for package "
                        f"'{pkg_name}' — cloak still present but locked. "
                        f"Refusing to leave the system in a dirty state."
                    )
                if not self.quiet:
                    safe_print(
                        _('   ℹ️  Cloak already gone (race-safe): {}').format(original_name)
                    )
                continue
        ...
        for original_path, cloak_path in cloaks_to_restore:
            try:
                ...
                if ok:
                    ...
                    # If the cloak file is gone, another process already restored it — fine.
                    # If it's still there, we cannot leave this as a locked hostage.
                    if cloak_path.exists():
                        raise RuntimeError(
                            f"HARD FAIL: Force-restore failed for '{original_path.name}' — "
                            f"cloak still present and locked. "
                            f"Cannot guarantee clean loader state. Resolve and retry."
                        )
                    if not self.quiet:
                        safe_print(
                            _('      ℹ️  Cloak already gone (race-safe): {}').format(original_path.name)
                        )
