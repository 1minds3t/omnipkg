 ...
 */
/* Embedded help text — generated by dev_tools/gen_help.py from help.toml.
 * Provides: OMNIPKG_HELP_TEXT (const char[]), OMNIPKG_VERSION (const char[])
 * Zero runtime IO — the string is baked into the binary at compile time. */
#include "_help.h"
...
int main(int argc, char **argv) {
    /* ── 0. Ultra-fast path: no-arg / --help / -h / --version / -v ──────────
     * Check argc / argv[1] before ANY other work — no realpath(), no getenv(),
     * no timer, no stat().  The only syscalls remaining after this block are
     * the ELF loader + one or two write() calls.
     * argv[1] is always set by the OS before main() is called; safe to read.
     *
     * argc == 1 (bare invocation, no command given):
     *   POSIX convention — error to stderr, help to stdout, exit non-zero.
     *   Mirrors uv (exit 2 = usage/argument error, same as argparse bad-arg exits)
     *   Cost: two write() syscalls (~1 µs total).  Still <350 µs end-to-end.
     *
     * --help / -h:
     *   Explicit help request — help to stdout, exit 0.
     *
     * --version / -v:
    ...
    if (argc >= 2) {
        ...
        /* ── Subcommand help: 8pkg <cmd> -h / --help ────────────────────────
         * argv[1] = subcommand name, argv[2] = -h or --help.
         * Look up OMNIPKG_HELP_<CMD> from the generated table; if found,
         * print it and exit 0 — zero Python startup, zero daemon contact. */
        if (argc >= 3) {
            const char *a2 = argv[2];
            if (strcmp(a2, "-h") == 0 || strcmp(a2, "--help") == 0) {
                /* Table of subcommand help strings baked in from help.toml. */
                static const struct { const char *name; const char *text; }
                sub_help[] = {
                    { "doctor",      OMNIPKG_HELP_DOCTOR      },
                    { "heal",        OMNIPKG_HELP_HEAL        },
                    { "install",     OMNIPKG_HELP_INSTALL     },
                    { "uninstall",   OMNIPKG_HELP_UNINSTALL   },
                    { "info",        OMNIPKG_HELP_INFO        },
                    { "swap",        OMNIPKG_HELP_SWAP        },
                    { "python",      OMNIPKG_HELP_PYTHON      },
                    { "config",      OMNIPKG_HELP_CONFIG      },
                    { "run",         OMNIPKG_HELP_RUN         },
                    { "daemon",      OMNIPKG_HELP_DAEMON      },
                    { "reset",       OMNIPKG_HELP_RESET       },
                    { "demo",        OMNIPKG_HELP_DEMO        },
                    { "stress-test", OMNIPKG_HELP_STRESS_TEST },
                    { "web",         OMNIPKG_HELP_WEB         },
                    { "export",      OMNIPKG_HELP_EXPORT      },
                    { "sync",        OMNIPKG_HELP_SYNC        },
                    { "revert",      OMNIPKG_HELP_REVERT      },
                    { NULL, NULL }
                };
                for (int si = 0; sub_help[si].name != NULL; si++) {
                    if (strcmp(a1, sub_help[si].name) == 0) {
                        fputs(sub_help[si].text, stdout);
                        return 0;
                    }
                }
                /* Unknown subcommand — fall through to Python for proper error. */
            }
        }
    }
...
#define DISPATCH_LOG_TIME(label) \
     ...
     */
    {
         ...
         * Read this FIRST: on the happy path (marker present, path embedded) we
         * get c_src for free and skip all filesystem candidate searching entirely.
        ...
        /* Path search — only needed when the marker is missing or had no path.
         * Candidates 1-3 are cheap stat() probes for common layouts.
         * Candidate 4 (dist-info glob) is the fallback for detached installs
         * like conda envs where source lives outside the venv tree; it costs
         * a glob() so we only run it when the cheaper candidates all miss. */
        if (!c_src[0]) {
            char cand[MAX_PATH];
            ...
            /* candidate 4: read direct_url.json from the dist-info in site-packages.
             * Works for any editable install (pip install -e .) regardless of where
             * the source tree lives — no env var needed, no Python invocation.
             * Only runs when marker is missing AND candidates 1-3 all missed,
             * so the glob cost is paid at most once (the self-seed below then
             * writes the marker and future runs skip this entire block). */
            if (!c_src[0]) {
                char venv4[MAX_PATH];
                dir_of(self_dir, venv4, sizeof(venv4));
                char du_pattern[MAX_PATH];
                snprintf(du_pattern, sizeof(du_pattern),
                         "%s/lib/python*/site-packages/omnipkg-*.dist-info/direct_url.json",
                         venv4);
                glob_t du_glob;
                memset(&du_glob, 0, sizeof(du_glob));
                if (glob(du_pattern, 0, NULL, &du_glob) == 0 && du_glob.gl_pathc > 0) {
                    FILE *du_f = fopen(du_glob.gl_pathv[0], "r");
                    if (du_f) {
                        char du_buf[MAX_PATH * 2];
                        size_t du_len = fread(du_buf, 1, sizeof(du_buf) - 1, du_f);
                        fclose(du_f);
                        du_buf[du_len] = '\0';
                        char url_val[MAX_PATH] = "";
                        json_get_str(du_buf, "url", url_val, sizeof(url_val));
                        const char *src_root = url_val;
                        if (strncmp(src_root, "file://", 7) == 0)
                            src_root += 7;
                        if (src_root[0]) {
                            snprintf(cand, sizeof(cand), "%s/src/omnipkg/dispatcher.c", src_root);
                            if (file_exists(cand))
                                strncpy(c_src, cand, sizeof(c_src) - 1);
                            if (!c_src[0]) {
                                snprintf(cand, sizeof(cand), "%s/omnipkg/dispatcher.c", src_root);
                                if (file_exists(cand))
                                    strncpy(c_src, cand, sizeof(c_src) - 1);
                            }
                        }
            ...
            /* Self-seed: marker was missing but we found the source — write the
             * marker now so every future run reads the path from it and skips
             * this entire search block. */
            if (c_src[0]) {
                FILE *wf = fopen(marker_path, "w");
                if (wf) {
                    fprintf(wf, "selfseeded:%s\n", c_src);
                    fclose(wf);
                    marker_found = 1;
                    if (debug)
                        fprintf(stderr, "[C-STALE] marker self-seeded → %s\n", marker_path);
                }
            }
        }
    ...
    if (argc >= 2) {
        ...
        /* daemon subcommands that must NEVER go through a daemon worker:
         *   restart / stop / kill — would deadlock (killing own parent process)
         *   monitor               — needs a real TTY for interactive input()
         *   idle                  — interactive config editor needs real TTY
         * Any "daemon <subcmd>" not in this list (e.g. "daemon status",
         * "daemon start") is safe to run through the daemon. */
        int is_daemon_direct = 0;
        if (argc >= 3 && strcmp(argv[1], "daemon") == 0) {
            const char *sub = argv[2];
            if (strcmp(sub, "restart") == 0 ||
                strcmp(sub, "stop")    == 0 ||
                strcmp(sub, "kill")    == 0 ||
                strcmp(sub, "monitor") == 0 ||
                strcmp(sub, "idle")    == 0) {
                is_daemon_direct = 1;
            }
        }
        ...
        is_interactive_command = ((is_info && !info_python) || is_config || is_logs_follow || is_heal || is_stress_test || is_demo || is_monitor || is_uninstall_interactive || is_daemon_direct);
    }
    if (!is_swap_python && !is_interactive_command) {
        ...
        if (is_swap_python)
            fprintf(stderr, "[C-DISPATCH] swap-python — running direct (no daemon)\n");
        else
            fprintf(stderr, "[C-DISPATCH] interactive/direct command — skipping daemon\n");
    }
    ...
    DISPATCH_LOG_TIME(is_interactive_command ? "pre-execv (interactive/direct)" : "pre-execv (daemon unavailable)");
