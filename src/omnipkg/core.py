...
class ConfigManager:
    ...
    def __init__(self, suppress_init_messages=False):
        ...
        if not setup_complete_flag.exists() and (not is_nested_interpreter):
            ...
            try:
                ...
                if not suppress_init_messages:
                    safe_print(
                        _("❌ A critical error occurred during initialization: {}").format(e)
                    )
        # 🎯 NEW: Lazy symlink verification (runs AFTER setup is complete)
        # Only check if we're not in a nested interpreter and setup was completed
        if setup_complete_flag.exists() and not is_nested_interpreter:
            self._verify_version_symlinks_lazy()
    ...
    def _ensure_omnipkg_bootstrapped(self, python_exe: Path, version: str):
        ...
        try:
            ...
            safe_print(_('   ✅ Successfully bootstrapped Python {}').format(version))
            return True
        except Exception as e:
    ...
    def _register_all_interpreters(self, venv_path: Path):
        ...
        with InterpreterManager._registry_write_lock:
                ...
                try:
                    ...
                    for version, path_str in existing_registry.get("interpreters", {}).items():
                        ...
                        if not str(path).lower().startswith(str(managed_interpreters_dir).lower()):
                            ...
                            # This path is inside .omnipkg/interpreters/
                            # Check if it's the SAME version as our current native Python
                            if version == current_version and ".omnipkg/interpreters" not in str(native_python):
                                # There's a downloaded version AND a native version of the same Python
                                # The native one should take priority
                                safe_print(
                                    _("   🔄 Found downloaded Python {} but native version exists - using native").format(version)
                                )
            ...
            if ".omnipkg/interpreters" not in str(current_exe):
                ...
                safe_print(
                    _("   ✅ Registered native Python {}: {}").format(
            ...
            all_interpreters = {**interpreters, **native_interpreters}  # Swap order!
            ...
            if all_interpreters:
                ...
                for version_str, path in interpreters.items():
                    if version_str not in native_interpreters:  # Only add if no native exists
                        all_interpreters[version_str] = path
            ...
            self._write_profile_script()   # co-atomic: inside the lock
            safe_print("   🔓 Released registry write lock")
        # lock released here
        if platform.system() != "Windows":
            self._create_version_symlinks()

    def _write_profile_script(self) -> None:
        """
        Atomically regenerate profile.d/omnipkg_pythons.sh from the current
        registry. Must be called any time the registry changes.
        
        Co-atomic with registry writes so the C dispatcher can trust these
        env vars as a ~100ns alternative to parsing registry.json (~20µs).
        """
        profile_dir = self.venv_path / ".omnipkg" / "profile.d"
        profile_dir.mkdir(parents=True, exist_ok=True)
        profile_path = profile_dir / "omnipkg_pythons.sh"
        ...
        try:
            ...
            return  # registry not ready yet, skip silently

        lines = ["# Auto-generated by omnipkg — do not edit manually.",
                "# Sourced by the venv activate script so every shell (including CI steps)",
                "# can resolve versioned commands without swap."]
        for version, path in sorted(interpreters.items()):
            key = "OMNIPKG_PYTHON_" + version.replace(".", "_") + "_PATH"
            # Normalise to X_Y style (3.11 → 311) to match C dispatcher lookup
            mm = version.replace(".", "")
            key_short = f"OMNIPKG_PYTHON_{mm}_PATH"
            lines.append(f'export {key_short}="{path}"')

        content = "\n".join(lines) + "\n"

        # Atomic write — temp file in same dir so os.replace is rename(), not copy
        import tempfile
        tmp_fd, tmp_path = tempfile.mkstemp(dir=profile_dir, suffix=".sh")
        try:
            with os.fdopen(tmp_fd, "w") as f:
                f.write(content)
            os.replace(tmp_path, profile_path)
        except Exception as e:
            safe_print(f"[DEBUG] _write_profile_script failed: {e}")
    ...
    def _install_essential_packages(self, python_exe: Path):
        ...
        safe_print(_('   📌 Target interpreter: Python {}').format(target_py_version))
        ...
        if system == "linux":
            ...
            if is_musl:
                ...
                if full_version.startswith("3.11") and full_version != "3.11.14":
                    safe_print(_('   🌲 Upgrading {} → 3.11.14 (musl-compatible)').format(full_version))
            ...
            try:
                ...
                if archive_name.endswith(".tar.zst"):
                    ...
                    try:
                        ...
                        safe_print(_('   - Current Context: Python {}').format(original_context))
                        ...
                        if omnipkg_instance.current_python_context != f"py{target_context}":
                            safe_print(
                                _('   - Swapping to Python {} to install dependency...').format(target_context)
                            )
                            if omnipkg_instance.switch_active_python(target_context) != 0:
                                raise OSError(
                                    _('Failed to switch to Python {} to install dependency.').format(target_context)
                                )
                        ...
                        try:
                            ...
                            safe_print(_('   - ⚠️  Automatic restart failed: {}').format(e))
    ...
    def _get_paths_for_interpreter(self, python_exe_path: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if not site_packages_path:
                raise RuntimeError(
                    _('No valid site-packages directory found in {} (platform: {})').format(sp_list, platform.system())
                )
            ...
            if is_already_managed:
                ...
                if not managed_python_exe.exists():
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            _('Could not create managed Python environment on Windows: {}').format(e)
                        ) from e
                else:
        ...
        for target in targets:
            ...
            if not shim_path.exists():
                ...
                if is_windows:
                    ...
                    try:
                        os.symlink(dispatcher_exe, shim_path)
    ...
    def _get_all_interpreters_for_config(self, current_version: str, current_path: Path) -> Dict[str, str]:
        """
        Helper to get a complete list of interpreters for the config file.
    ...
    def _ensure_all_interpreters_have_config(self):
        """
        Ensure all registered Python interpreters have .omnipkg_config.json files.
        This should be called after adopting a new Python or during initialization.
        """
        ...
        for version, path_str in interpreters.items():
            ...
            if path.exists():
                ...
                if not config_path.exists():
                    ...
                    self._write_interpreter_config(path, version)
    ...
    def _load_or_create_env_config(self, interactive: bool = True) -> Dict:
        ...
        return self._load_or_create_config(interactive)
...
class BubbleIsolationManager:
                    ...
                    if self.parent_omnipkg.cache_client:
                        try:
                            ...
                            if target_dist:
                                ...
                                deps_raw = enriched.get("resolved_deps", "{}")
    ...
    def create_bubble_from_main_env(
        ...
        for item in site_packages.iterdir():
            ...
            if item.name in skip_items:
                safe_print(_('   - Skipping: {}').format(item.name))
                continue
            ...
            try:
                ...
                safe_print(_('   - Warning: Could not copy {}: {}').format(item.name, e))
                continue
    ...
    def install_and_verify(
        ...
        # Tell uv-ffi what's leaving before we nuke the dir,
        # so BUBBLE_SITE_PACKAGES_CACHE stays coherent and the next
        # install doesn't hit a cold 27ms resolve.
        # After return_code == 0, add:
        try:
        ...
        # Install directly to the final bubble destination — no staging needed.
        ...
        # 2. FINALIZE BUBBLE - already in place, nothing to move
    ...
    def _heal_corrupt_metadata(self, install_path: Path, expected_package_name: str):
        ...
        if healed_count > 0:
            safe_print(_('   ✅ Healed {} corrupt metadata file(s)').format(healed_count))
    ...
    def create_bubble_for_package(
        ...
        if package_name == "omnipkg":  # Special handling for omnipkg itself
            ...
            if project_root and (project_root / "pyproject.toml").exists():
                ...
                is_editable = True
                safe_print(_('   - Detected development environment: {}').format(install_source))
            ...
            try:
                ...
                if direct_url_json:
                    ...
                    if project_root and (project_root / "pyproject.toml").exists():
                        install_source = str(project_root)
                        is_editable = True
                        safe_print(_('   - Detected local development source: {}').format(install_source))
            except (
    ...
    def _create_bubble_from_editable_install(
        ...
        try:
            ...
            safe_print(_('   - Using site-packages: {}').format(site_packages))
            ...
            if pyproject_path.exists():
                try:
                    ...
                    safe_print(_('   - Found {} dependencies in pyproject.toml').format(len(dependencies)))
                except Exception as e:
                    safe_print(_('   - ⚠️  Could not parse pyproject.toml: {}').format(e))
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                safe_print(_('   - Copying source files from: {}').format(package_dir))
                shutil.copytree(
                    package_dir,
                    temp_pkg_dir,
                    symlinks=False,
                ...
                for dist_info_pattern in [
                    ...
                    for dist_info in site_packages.glob(dist_info_pattern):
                        if package_name in dist_info.name or "editable" in dist_info.name:
                            ...
                            safe_print(_('   - Copied metadata: {}').format(dist_info.name))
                ...
                if not dist_info_found:
                    ...
                    metadata_file.write_text(
                        _('Metadata-Version: 2.1\nName: {}\nVersion: {}\n').format(package_name, version)
                    )

                # 3. Copy ALL dependencies from site-packages
                safe_print(_('   - Copying {} dependencies...').format(len(dependencies)))
            ...
            try:
                ...
                if metadata_file.exists():
                    ...
                    if "Name:" not in content[:500]:
                        ...
                        safe_print(_('   🔧 Healed missing Name in {}').format(dist_info.name))
                # ==========================================
        ...
        for candidate in import_candidates:
            script_lines.extend(
                [
                    ...
                    _("    results.append(('{}', True, version))").format(candidate),
                    "except Exception as e:",
                    _("    results.append(('{}', False, str(e)))").format(candidate),
                ]
            )
            ...
            for donor_file in donor_pkg_dir.rglob("*.py"):
                ...
                if not target_file.exists():
                    ...
                    try:
                        ...
                        safe_print(_('      ⚠️ Transplant failed: {}').format(e))
    ...
    def _fix_bubble_import_failures(
        ...
        for failure in import_failures:
            ...
            safe_print(_('   🔍 Healing {}...').format(pkg_name))
            ...
            if import_success["importable"]:
                safe_print(_('      ✅ Successfully healed {}').format(pkg_name))
            else:
        ...
        try:
            ...
            if ".dist-info" in str(source_path) and source_path.name in (
                ...
                if not source_path.exists() or source_path.stat().st_size == 0:
                    raise IOError(_('Source metadata file is empty or missing: {}').format(source_path))
            ...
            if ".dist-info" in str(source_path) and source_path.name == "METADATA":
                if (
                    ...
                    raise IOError(_('METADATA file copy verification failed: {}').format(target_path))

        except Exception as e:
            safe_print(_('    ❌ Failed to copy {}: {}').format(source_path.name, e))
            raise
    ...
    def _create_bubble_manifest(
        ...
        if enriched_meta:
            try:
                ...
                if enriched_meta.get("resolved_deps"):
                    deps = enriched_meta.get("resolved_deps")
...
class omnipkg:
                    ...
                    if not original_path.exists():
                        ...
                        try:
                            if cloak_path.is_dir():
                                ...
                                cloak_path.unlink()
                            total_cleaned += 1
                            safe_print(_('         🗑️  Deleted duplicate cloak: {}').format(cloak_path.name))
                        except Exception as e:
    ...
    def _perform_concurrent_healing(
        ...
        if "--verbose" in sys.argv or "-V" in sys.argv:
            safe_print(_('   🔍 Current exe: {}').format(current_exe))
            safe_print(_('   🔍 Native exe:  {}').format(native_exe))
            safe_print(_('   🔍 Is native:   {}').format(is_current_native))
            safe_print(_('   🔍 Current version: {}').format(current_version))
        ...
        if _native_already_synced:
            ...
            try:
                ...
                if current_version_str != master_version:
                    ...
                    if "--verbose" in sys.argv or "-V" in sys.argv:
                        safe_print(
                            _('   🔍 Native needs sync: {} -> {}').format(current_version_str, master_version)
                        )
            except Exception as e:
                native_needs_sync = True
                if "--verbose" in sys.argv or "-V" in sys.argv:
                    safe_print(_('   🔍 Native sync check failed: {}').format(e))
        else:
    ...
    def initialize_pypi_cache(self):
        ...
        safe_print(_('📊 PyPI Contextual Cache initialized: {} valid entries.').format(stats['valid_entries']))
        ...
        if self.config.get("redis_enabled", True):
            ...
            if not REDIS_AVAILABLE:
                ...
                try:
                    ...
                    safe_print(_('   - Error: {}: {}').format(type(e).__name__, e))
                    safe_print("   - Auto-switching to SQLite for this session")
    ...
    def _handle_redis_fallback(self):
        ...
        try:
            ...
            safe_print(_('   ⚠️  Could not persist Redis fallback to config: {}').format(e))
            self.config["redis_enabled"] = False
    ...
    def _repair_manifest_context_mismatch(
        ...
        try:
            ...
            if manifest_version and manifest_version != current_python_version:
                safe_print(_("   ⚠️  FIXING: Detected manifest mismatch in '{}'.").format(bubble_root_name))
                safe_print(
                ...
                safe_print(
                    _('      - ✅ Repaired manifest to claim Python {}.').format(current_python_version)
                )
        ...
        with self.cache_client.pipeline() as pipe:
            for dist in instances_to_clone:
                try:
                    ...
                    safe_print(_('      - ⚠️  Failed to fast-clone {}: {}').format(dist.metadata['Name'], e))
                    ...
                    try:
                        ...
                        if metadata_file.exists():
                            ...
                            if len(parts) >= 1:
                                ...
                                if "Name:" not in content[:500]:
                                    safe_print(_('   🔧 HEALING corrupt metadata: {}').format(dist_info.name))
                                    fixed_content = _('Name: {}\n{}').format(inferred_name, content)
    ...
    def _synchronize_knowledge_base_with_reality(
        ...
        for pkg_name in sorted(list(all_package_names)):
            ...
            if disk_version != kb_version:
                ...
                if verbose:
                    ...
                    safe_print(_('     - In Knowledge Base     : v{}').format(kb_version or 'Not Set'))

                main_key = f"{self.redis_key_prefix}{pkg_name}"
                if disk_version:
                    safe_print(
                        _("   - 🔧 REPAIRING KB: Setting '{}' active version to v{}.").format(pkg_name, disk_version)
                    )
        ...
        if ghosts_in_kb:
            safe_print(_('   -> 👻 Found {} ghost package(s) to exorcise...').format(len(ghosts_in_kb)))
    ...
    def _validate_instance_integrity(
        ...
        if instance_data.get("Version") != version:
            issues.append(
                _('Version mismatch: KB says {}, expected {}').format(instance_data.get('Version'), version)
            )
            ...
            if dist_info_path.exists() and dist_info_path.is_dir():
                ...
                if self._is_ghost_dist_info(dist_info_path, pkg_name, c_name):
                    try:
                        ...
                        safe_print(_('    -> ⚠️  Failed to remove {}: {}').format(dist_info_name, e))
    ...
    def _clean_corrupted_installs(self):
        ...
        for item in all_corrupted_items:
            try:
                safe_print(_('    -> 💀 Removing corrupted: {}').format(item.name))
                if item.is_dir():
                    shutil.rmtree(item)
                else:
                    item.unlink()
                ...
                try:
                    ...
                    safe_print(_('    -> ⚠️  Could not remove {}: {}').format(item.name, e))

        if cleanup_count > 0:
            safe_print(_('🎉 Cleaned {} corrupted installations').format(cleanup_count))
    ...
    def _hunt_and_exorcise_all_ghosts(self, site_packages_path=None, live_active_versions=None):
        ...
        for dist_info_path in ghosts_found:
            try:
                safe_print(_('    -> 👻 Found ghost: {}').format(dist_info_path.name))
                shutil.rmtree(dist_info_path)
                ...
                safe_print(_('    -> ⚠️  Failed to remove ghost {}: {}').format(dist_info_path.name, e))
    ...
    def heal(self, dry_run: bool = False, force: bool = False) -> int:
        ...
        try:
            ...
            safe_print(_('❌ An unexpected error occurred during the audit: {}').format(e))
            return 1
        ...
        for pkg_name, specs in grouped_reqs.items():
            safe_print(_("   - Reconciling '{}':").format(pkg_name))
            ...
            for spec_str in specs:
                ...
                if match:
                    ...
                    if part:
                        ...
                        safe_print(_('     - Found constraint: {}').format(part))
            ...
            safe_print(_("     - Combined spec: '{}'").format(combined_spec))
            ...
            if resolved_spec_str:
                safe_print(_('   - ✅ Elected Winner via intersection: {}').format(resolved_spec_str))
                ...
                safe_print(_("     - Failed to resolve: '{}'").format(combined_spec))
    ...
    def _save_last_known_good_snapshot(self, known_state=None, target_directory=None):
        """
        ...
        if not target_directory:  # don't snapshot FFI state for --target installs
            try:
                from omnipkg._vendor.uv_ffi import get_site_packages_cache
                _pairs = get_site_packages_cache()
                if _pairs:
                    current_state = {canonicalize_name(name): ver for name, ver in _pairs}
                    ...
                    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as _s:
                        ...
                        if _state:
                            if isinstance(_state, list):
                                current_state = {canonicalize_name(n): v for n, v in _state}
                            elif isinstance(_state, dict):
                                current_state = {canonicalize_name(k): v for k, v in _state.items()}
                            if current_state:
    ...
    def adopt_interpreter(self, version: str) -> int:
        ...
        def _post_adopt(python_exe: Path) -> None:
            ...
            self._push_c_binary_to_adopted_bin(python_exe)
            self.config_manager._write_profile_script()   # <-- add this
    ...
    def _is_interpreter_directory_valid(self, path: Path) -> bool:
        ...
        """
        safe_print(_('   🔍 DEBUG: Checking interpreter validity at: {}').format(path))
        if not path.exists():
            safe_print(_('   ❌ DEBUG: Path does not exist: {}').format(path))
            return False
        safe_print(_('   ✓ DEBUG: Path exists'))
        bin_dir = path / "bin"
        if bin_dir.is_dir():
            for name in [
                "python3.14",
            ...
            with FileLock(lock_path, timeout=300):
                ...
                if not dest_path.exists():
                    ...
                    for candidate in sorted(interpreters_dir.glob(f"cpython-{major_minor}.*"), reverse=True):
                        if candidate.is_dir():
                            safe_print(_('   - Found candidate: {}').format(candidate))
                            if self._is_interpreter_directory_valid(candidate):
                                dest_path = candidate
                                safe_print(_('   - ✅ Using upgraded installation at: {}').format(dest_path))
                                break
                ...
                if dest_path.exists() and self._is_interpreter_directory_valid(dest_path):
                    ...
                    safe_print(_('   - Checked path: {}').format(dest_path))
                    safe_print(_('   - Path exists: {}').format(dest_path.exists()))
                    return 1
        ...
        try:
            ...
            if result.returncode == 0 and pip_report_str:
                ...
                try:
                    ...
                    safe_print(_('   ⚠️  Extracted JSON (first 500 chars): {}').format(json_str[:500]))
                    return None, pip_user_error_output
            ...
            if is_no_match_error:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        safe_print(
                            _('      - Target Version: {}, Requires Python: {}').format(version_to_check, compatible_py or 'unknown')
                        )
    ...
    def _restore_from_pre_install_snapshot(self, snapshot_key: str) -> bool:
        ...
        if not snapshot_data:
            safe_print(_('   ❌ Snapshot not found: {}').format(snapshot_key))
            return False
    ...
    def _safe_restore_from_snapshot(
        ...
        safe_print(_('   Reason: Failed to bubble {}, must restore stability').format(package_name))
        ...
        if to_downgrade:
            ...
            for pkg, (current, target) in sorted(to_downgrade.items()):
                safe_print(_('      • {}: v{} → v{}').format(pkg, current, target))
        ...
        if to_downgrade:
            ...
            safe_print(
                _('\n   🔧 Restoring {} package(s) to snapshot versions...').format(len(specs_to_restore))
            )
            ...
            if restore_code != 0:
                ...
                safe_print(_('   📄 Pip output:\n{}').format(restore_output.get('stderr', 'No error output')))
                success = False

        # Then uninstall packages that shouldn't exist
        if success and to_uninstall:
            safe_print(_('\n   🗑️  Removing {} unexpected package(s)...').format(len(to_uninstall)))
        ...
        if success:
            ...
            for pkg, expected_ver in snapshot_state.items():
                ...
                if actual_ver != expected_ver:
                    mismatches.append(
                        _('{}: expected v{}, got v{}').format(pkg, expected_ver, actual_ver or 'MISSING')
                    )

            if mismatches:
                safe_print(_('\n   ⚠️  Restore incomplete - {} mismatches:').format(len(mismatches)))
    ...
    def smart_install(self, packages, dry_run=False, force_reinstall=False,
                  override_strategy=None, target_directory=None,
                  preflight_compatibility_cache=None, index_url=None,
                  extra_index_url=None, extra_flags=None) -> int:
        from omnipkg.installation.smart_install import SmartInstaller
        return SmartInstaller(self).install(
            packages=packages,
            dry_run=dry_run,
            force_reinstall=force_reinstall,
            override_strategy=override_strategy,
            target_directory=target_directory,
            preflight_compatibility_cache=preflight_compatibility_cache,
            index_url=index_url,
            extra_index_url=extra_index_url,
    ...
    def smart_upgrade(
        ...
        if not current_version_str:
            ...
            return 1
        safe_print(_('   - Current installed version: {}').format(current_version_str))
        ...
        if not target_version_str:
            ...
            return 1
        safe_print(_('   - Target PyPI version: {}').format(target_version_str))

        if current_version_str == target_version_str and not force:
            safe_print(_('✅ Already on version {}. No upgrade needed.').format(target_version_str))
            return 0
        ...
        safe_print(
            _('\n   - Step 1: Preserving current version (v{}) in a bubble...').format(current_version_str)
        )
        if not self.bubble_manager.create_bubble_for_package(
            "omnipkg", current_version_str, python_context_version=python_context
        ):
            safe_print(
                _('   - ❌ Failed to bubble current version v{}. Aborting.').format(current_version_str)
            )
            return 1
        ...
        safe_print(_('   - ✅ Successfully bubbled and indexed omnipkg v{}.').format(current_version_str))
        ...
        try:
            ...
            safe_print(_('\n   - ❌ CRITICAL: Handover failed: {}').format(e))
            return 1
    ...
    def _create_pypi_only_bubble(
        ...
        install_source = f"{package_name}=={version}"
        safe_print(_('   - Using PyPI source: {}').format(install_source))
    ...
    def _find_compatible_python_version(
        ...
        try:
            ...
            safe_print(_('   - 📍 Current Python: {}').format(current_py_str))
            ...
            if target_package_version:
                ...
                safe_print(
                    _('   - 🎯 Checking compatibility for: {}=={}').format(package_name, target_package_version)
                )
            else:
            ...
            if supported_py_versions:
                safe_print(
                    _('   - Found Python versions supported by wheels: {}').format(', '.join(sorted(supported_py_versions)))
                )
                ...
                if any(current_py_tuple == v_tuple for v_tuple in supported_tuples):
                    safe_print(
                        _('   - ✅ Current Python {} is compatible according to wheel data.').format(current_py_str)
                    )
            ...
            safe_print(_("   - 📋 requires_python metadata: '{}'").format(requires_python))
            ...
            safe_print(
                _('   - ✅ Current Python {} seems compatible based on metadata.').format(current_py_str)
            )
        ...
        try:
            ...
            for pkg_name, active_version in final_active_packages.items():
                ...
                if bubble_path.exists() and bubble_path.is_dir():
                    ...
                    try:
                        # Read manifest BEFORE deletion to know what packages to evict
                        manifest_path = bubble_path / ".omnipkg_manifest.json"
                        packages_in_bubble = []
                        if manifest_path.exists():
                            import json as _json
                            ...
                            packages_in_bubble = list(_manifest.get("packages", {}).keys())
                            safe_print(f"   - [DEBUG] manifest packages: {packages_in_bubble!r}")
                            # fallback: parse from dir names if manifest missing pkg list
                        if not packages_in_bubble:
                            packages_in_bubble = [
                                d.name.split("-")[0]
                                for d in bubble_path.iterdir()
                                if d.suffix == "" and d.is_dir() and not d.name.startswith(".")
                            ]
                        try:
                        ...
                        # Bubble cache is a single slot keyed to the ACTIVE bubble —
                        # do not touch it here. Deleting a redundant bubble (identical
                        # to main env active version) has no effect on the active bubble
                        # cache. Evicting would nuke the wrong thing.
                        safe_print(f"   - 🧠 Skipping bubble cache eviction (not the active bubble)")
                    except Exception as e:
            ...
            safe_print(_('   - ⚠️  An error occurred during bubble cleanup: {}').format(e))
    ...
    def _backup_corrupted_file(self, file_path: str, backup_base_dir: Optional[str] = None) -> bool:
        ...
        try:
            ...
            safe_print(_('   - 💾 Backed up corrupted file to: {}').format(backup_path))
            return True

        except Exception as e:
            safe_print(_('   - ⚠️  Failed to backup {}: {}').format(file_path, e))
            return False
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt < max_attempts - 1:  # Don't heal on the last attempt
                    safe_print(
                        _('\n🛡️  AUTO-HEAL: Detected corruption (attempt {}/{})').format(attempt + 1, max_attempts)
                    )
                    safe_print(_('   - 💀 Corrupted file: {}').format(os.path.basename(corrupted_file)))
                    ...
                    if self._backup_corrupted_file(corrupted_file):
                        try:
                            ...
                            safe_print(_('   - ❌ Failed to remove corrupted file: {}').format(e))
                            return proc
                else:
                    safe_print(
                        _('❌ Max repair attempts ({}) reached. Manual intervention needed.').format(max_attempts)
                    )
                    return proc
                ...
                safe_print(_('❌ Error running conda command: {}').format(e))
                raise
    ...
    def _heal_conda_environment(self, also_run_clean: bool = True):
        ...
        # Healing process
        safe_print(_('   - 💀 Found {} corrupted file(s)').format(len(corrupted_files_found)))
        ...
        for corrupted_file in corrupted_files_found:
            ...
            safe_print(_('      -> Processing: {}').format(file_name))
        ...
        if cleaned_count > 0:
            safe_print(_('   - 🧹 Successfully cleaned {} corrupted file(s)').format(cleaned_count))
            ...
            if also_run_clean:
                ...
                try:
                    ...
                    if clean_proc.returncode == 0:
                        ...
                        safe_print(_('      ⚠️  Conda clean had issues: {}').format(clean_proc.stderr))
                except Exception as e:
                    safe_print(_('      ❌ Error running conda clean: {}').format(e))
    ...
    def safe_conda_command(self, cmd_args: List[str], max_heal_attempts: int = 2) -> bool:
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                safe_print(_('❌ Conda command failed: {}').format(proc.stderr))
                return False

        except Exception as e:
            safe_print(_('❌ Exception during conda command: {}').format(e))
            return False
            ...
            for item in final_to_uninstall:
                ...
                # Check if the path is a valid string before trying to use it
                    ...
                    if bubble_dir.name in (expected_bubble_name, expected_bubble_name_alt, expected_bubble_name_underscored) and bubble_dir.is_dir():
                        ...
                        safe_print(_('      - Expected Bubble Name: {}').format(expected_bubble_name))
                        safe_print(_('      - Found Bubble Name:    {}').format(bubble_dir.name))
                        safe_print(_('      - Path Being Checked:   {}').format(bubble_dir))
                # --- END OF FIX ---
                else:
    ...
    def _clear_windows_module_cache(self):
        ...
        if modules_cleared:
            safe_print(
                _('   📦 Cleared {} cached modules: {}').format(len(modules_cleared), ', '.join(modules_cleared))
            )
    ...
    def _find_best_version_for_spec(self, package_spec: str) -> Optional[str]:
        ...
        try:
            ...
            if ";" in package_spec:
                ...
                safe_print(_("    -> Detected environment marker: '{}'").format(marker_str))
                ...
                try:
                    ...
                    if not marker.evaluate():
                        ...
                        safe_print(_('    -> Marker: {}').format(marker_str))
                        safe_print(_('    -> Current Python: {}').format(self.current_python_context))
                        # Could return None here, or continue anyway - your choice
                        # For now, let's continue and try to resolve anyway
                except Exception as e:
                    safe_print(_('    ⚠️  Could not evaluate environment marker: {}').format(e))
                        ...
                        if requires_python:
                            ...
                            if min_py:
                                ...
                                raise NoCompatiblePythonError(
                                    ...
                                    message=_("Package '{}' requires Python {}, current is {}").format(pkg_name, min_py, self.current_python_context),
                                )
                except NoCompatiblePythonError:
                    raise
                except Exception as e:
                    safe_print(_('    ⚠️ Could not determine compatible Python version: {}').format(e))

                return None

            # 3. Now check if this version satisfies the original constraint
            safe_print(_('    ✅ Pip found compatible version: {}').format(test_result))
            specifier = SpecifierSet(spec_str)

            if not specifier.contains(test_result):
                safe_print(
                    _("    ⚠️ Compatible version {} does NOT satisfy constraint '{}'").format(test_result, spec_str)
                )
                safe_print(
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if valid_versions:
                            ...
                            safe_print(
                                _("    💡 Latest version matching '{}': {}").format(spec_str, latest_matching)
                            )
                            ...
                            if min_python_from_wheels:
                                ...
                                raise NoCompatiblePythonError(
                                    ...
                                    message=_("Package '{}' requires Python {}, current is {}").format(pkg_name, compatible_python_clean, self.current_python_context),
                                )
                except NoCompatiblePythonError:
                    raise
                except Exception as e:
                    safe_print(_('    ⚠️ Could not determine required Python version: {}').format(e))

                return None

            # 4. Success! We have a compatible version that satisfies the constraint
            resolved_spec = f"{pkg_name}=={test_result}"
            safe_print(_("    ✅ Resolved '{}' to '{}'").format(package_spec, resolved_spec))
            ...
            safe_print(_("    ❌ Failed to resolve complex specifier '{}': {}").format(package_spec, e))
            return None
    ...
    def _resolve_package_versions(self, packages: List[str]) -> List[str]:
        ...
        for pkg_spec in packages:
            safe_print(_("🔍 DEBUG: Processing '{}'").format(pkg_spec))
            ...
            if any(op in pkg_spec for op in complex_spec_chars):
                ...
                if resolved:
                    ...
                    safe_print(_("    ⚠️  Could not resolve '{}'. Skipping.").format(pkg_spec))
                continue
            ...
            if target_version:
                ...
                raise ValueError(_("Package '{}' not found or could not be resolved.").format(pkg_name))
...
"""
        ...
        try:
            ...
            try:
                ...
                safe_print(
                    _('   -   Error: {}: {}').format(error_data.get('exception_type'), error_data.get('exception_message'))
                )
            except (json.JSONDecodeError, KeyError):
                ...
                safe_print(_('   -   STDERR: {}').format(e.stderr.strip()))
    ...
    def _run_post_install_import_test(
        ...
        try:
            ...
            if main_package_imported:
                safe_print(_('      ✅ Successfully imported main package: {}').format(main_import_name))
                return True
            else:
                safe_print(_("      ❌ Main package '{}' failed to import").format(main_import_name))
                return False
    ...
    def _execute_historical_install(
        ...
        safe_print(
            _('      - 🖥️  Platform: Python {} on {} ({})').format(platform_info['py_version'], platform_info['system'], ', '.join(platform_info['platform_tags'][:2]))
        )
        safe_print(_('      - 🎯 Install Target: {}').format(install_target_desc))
        ...
        for pkg_name, pkg_ver in historical_versions.items():
            ...
            try:
                ...
                safe_print(_('      - 📥 Downloading {}...').format(filename))
                ...
                if return_code != 0:
                    safe_print(_('      - ❌ Failed to install {}=={}').format(pkg_name, pkg_ver))
                    self._restore_modern_setuptools()
                    return False

            except Exception as e:
                safe_print(_('      - ❌ Error processing {}=={}: {}').format(pkg_name, pkg_ver, e))
                self._restore_modern_setuptools()
                return False
        ...
        if itsdangerous_file:
            ...
            safe_print(
                _('      - 📥 Downloading {} (installing LAST to ensure single-file version)...').format(filename)
            )
        ...
        # Stage 3
        safe_print(_('\n      - ⚙️ Stage 3: Building {}=={}...').format(target_pkg, target_ver))
    ...
    def _get_historical_release_date(self, package_name, version):
        ...
        try:
            ...
            safe_print(_('      - ✓ Found release date: {}').format(upload_time))
            return upload_time
    ...
    def _run_pip_install(self, packages, force_reinstall=False,
        ...
        return ModularInstaller(self).install(
            packages=packages, force_reinstall=force_reinstall,
            target_directory=target_directory, extra_flags=extra_flags,
    ...
    def _find_newest_version_for_python(
        ...
        try:
            ...
            safe_print(_('    ⚠️  Error finding compatible version: {}').format(e))
            return None
    ...
    def _get_latest_version_from_pypi(
        ...
        try:
            ...
            if test_result:
                safe_print(_('    🎯 Found compatible version: {}').format(test_result))
                    ...
                    if requires_python:
                        ...
                        if not spec.contains(current_py_str):
                            ...
                            if pkg_min_python:
                                ...
                                safe_print(
                                    _('    ⚠️  Incompatible: {} v{} requires {}').format(package_name, target_version, pkg_min_python)
                                )
                                ...
                                raise NoCompatiblePythonError(
                                    ...
                                    message=_("Package '{}' v{} requires Python {}, current is {}").format(package_name, target_version, pkg_min_python, py_context),
                                )
                ...
                if result_check.returncode == 0:
                    ...
                    if version_match:
                        ...
                        if installed_version == latest_pypi_version:
                            safe_print(
                                _('    🚀 JACKPOT! Latest PyPI version {} is already installed!').format(latest_pypi_version)
                            )
                            safe_print(
        ...
        if latest_pypi_version:
            ...
            try:
                ...
                if compatible_version:
                    safe_print(
                        _('    🎯 Found compatible version {} - caching and returning!').format(compatible_version)
                    )
                    # Cache the result and return - FIX: Include python_context
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                for pattern in error_patterns:
                    if re.search(pattern, output_to_search, re.IGNORECASE):
                        safe_print(
                            _("    ❌ Package '{}' appears to not exist or be accessible").format(package_name)
                        )
                        ...
                        if package_exists_on_pypi and latest_pypi_version:
                            ...
                            if compatible_py_version:
                                safe_print(
                                    _('    ✅ Found compatible Python version: {}').format(compatible_py_version)
                                )
                                safe_print(
                            ...
                            raise NoCompatiblePythonError(
                                ...
                                message=_("Package '{}' v{} requires Python {}, but current Python is {}").format(package_name, latest_pypi_version, compatible_py_version, py_context),
                            )
                        else:
            ...
            already_satisfied_patterns = [
                _('Requirement already satisfied:\\s+{}\\s+in\\s+[^\\s]+\\s+\\(([^)]+)\\)').format(re.escape(package_name)),
                _('Requirement already satisfied:\\s+{}==([^\\s]+)').format(re.escape(package_name)),
                _('Requirement already satisfied:\\s+{}-([^\\s]+)').format(re.escape(package_name)),
            ]
            ...
            patterns = [
                _('(?:Would install|Installing collected packages:|Collecting)\\s+{}-([0-9]+(?:\\.[0-9]+)*(?:[a-zA-Z0-9\\.-_]*)?)').format(re.escape(package_name)),
            ...
            if package_exists_on_pypi and latest_pypi_version:
                ...
                if compatible_py_version:
                    safe_print(
                        _(' ✅ {} v{} requires Python {}').format(package_name, latest_pypi_version, compatible_py_version)
                    )
                else:
                    safe_print(" ⚠️  Could not determine compatible Python version")
                    compatible_py_version = "unknown"
                ...
                raise NoCompatiblePythonError(
                    ...
                    current_python=py_context,
                    compatible_python=compatible_py_version,
                    message=_("Package '{}' v{} requires Python {}, current is {}").format(package_name, latest_pypi_version, compatible_py_version, py_context),
                )
            else:
            ...
            safe_print(_(' ❌ An unexpected error occurred: {}').format(e))
            return None
    ...
    def list_packages(self, pattern: str = None) -> int:
        ...
        for pkg in packages_data:
            ...
            active_str = _('✅ {}').format(pkg['active']).ljust(20) if pkg["active"] else "(none)".ljust(20)
            ...
            if pkg["bubbled"]:
                ...
                if len(pkg["bubbled"]) > 3:
                    bubbled_str += _(', ... ({} more)').format(len(pkg['bubbled']) - 3)
    ...
    def show_multiversion_status(self) -> int:
        ...
        # ── 0. KB sync — streams live progress bars, heals env ───────────────
        if not self._connect_cache():
            return 1
        self._synchronize_knowledge_base_with_reality(verbose=True)

        # ── 1. Header (print immediately) ────────────────────────────────────
        sep = "=" * 60
        strategy = self.config.get("install_strategy", "stable-main")
        safe_print("\n" + sep)
        safe_print(_("  🚀 omnipkg   {}   strategy: {}").format(
            "{}.{}".format(sys.version_info.major, sys.version_info.minor), strategy
        ))
        safe_print(sep)

        # ── 2. Daemon status (socket + process-level resource monitor) ────────
        daemon_line = _("💤 not running   — start with '8pkg daemon start'")
        try:
            from omnipkg.isolation.worker_daemon import WorkerPoolDaemon, DaemonClient
            if WorkerPoolDaemon.is_running():
                # socket stats: req counts, errors
                result = DaemonClient().status()
                reqs   = result.get("stats", {}).get("total_requests", 0) if result.get("success") else 0
                errs   = result.get("stats", {}).get("errors", 0) if result.get("success") else 0

                # process-level stats: RAM, GPU, active vs idle
                try:
                    from omnipkg.isolation.resource_monitor import (
                    ...
                    for proc in processes:
                        ...
                        if wt == "DAEMON_MANAGER" or wt == "OTHER":
                            continue
                        ram_mb = proc["rss"] / 1024
                        total_ram_mb += ram_mb
                        total_gpu_mb += proc["gpu_mb"]
                        if wt.startswith("IDLE_WORKER"):
                            idle_count += 1
                        else:
                            active_count += 1

                    gpu_str = _("   {} MB VRAM").format(total_gpu_mb) if total_gpu_mb else ""
                    err_str = _("   {} errs").format(errs) if errs else ""
                    daemon_line = _(
                        "✅ running   {} active   {} idle   {:.0f} MB RAM{}{}   {} reqs"
                    ).format(
                        active_count, idle_count, total_ram_mb, gpu_str, err_str, reqs
                    )
                except Exception:
                    # resource monitor unavailable, fall back to socket-only
                    mem = result.get("memory_percent", -1) if result.get("success") else -1
                    idle_total = sum(result.get("idle_pool_info", {}).values()) if result.get("success") else 0
                    mem_str = _("   {:.1f}% mem").format(mem) if mem >= 0 else ""
                    daemon_line = _("✅ running   {} warm workers   {} reqs{}").format(
                        idle_total, reqs, mem_str
                    )
            else:
                daemon_line = _("💤 not running   — start with '8pkg daemon start'")
        except Exception:
            daemon_line = _("⚠️  status unavailable")
        safe_print("")
        safe_print(_("  Daemon    {}").format(daemon_line))

        # ── 3. Jailed tools (fast subprocess, print immediately) ──────────────
        jail_parts = []
        for tool in ("pip", "uv", "conda"):
            try:
                import subprocess as _sp
                r = _sp.run([tool, "--version"], capture_output=True, text=True, timeout=5)
                if r.returncode == 0:
                    tokens = r.stdout.strip().split()
                    ver = tokens[1] if len(tokens) >= 2 else tokens[0]
                    jail_parts.append("{} {}".format(tool, ver))
            except Exception:
                pass
        if jail_parts:
            safe_print(_("  Jail      🔒 {}").format("   ".join(jail_parts)))

        # ── 4. Lock freshness (read existing file, print immediately) ─────────
        lock_line = _("⚠️  none — run '8pkg export' to create one")
        try:
        ...
        if _tl is not None:
            try:
                from omnipkg.integration.reproducible import _venv_root as _get_venv_root
                _lock_path = _get_venv_root() / ".omnipkg" / "omnipkg.lock"
                if _lock_path.exists():
                    with open(_lock_path, "rb") as _fh:
                        _existing = _tl.load(_fh)
                    _sha = _existing.get("meta", {}).get("content_sha", "")
                    _gen = _existing.get("meta", {}).get("generated", "")
                    try:
                        from datetime import datetime, timezone as _tz
                        _dt = datetime.fromisoformat(_gen).astimezone(_tz.utc)
                        _ts = _dt.strftime("%Y-%m-%d %H:%M UTC")
                    except Exception:
                        _ts = _gen[:16].replace("T", " ")
                    lock_line = _("📄 {}   sha:{}...").format(_ts, _sha[:8] if _sha else "?")
            except Exception:
                pass
        safe_print(_("  Lock      {}").format(lock_line))

        # ── 5. In-memory env snapshot — streams [ver] source: lines live ──────
        safe_print("")
        safe_print(_("  Python environments:"))
        sys.stdout.flush()

        lock_data = None
        try:
            from omnipkg.integration.reproducible import (
                _venv_root, _load_registry, _load_interp_config,
                _detect_env_identity, _try_redis_for_python,
                _scan_active_from_fs, _scan_bubbles_from_fs, _omnipkg_version,
            )
            # do NOT redirect stderr here — let "[3.11] source: ..." stream live
            ...
            for ver, py_path_str in sorted(interpreters.items()):
                ...
                cfg        = _load_interp_config(py_path.parent)
                ...
                python_data[ver] = {"active": active, "bubbles": bubbles}

                # print each python row as soon as its data is ready
                n_active  = len(active)
                n_bubbles = sum(len(v) for v in bubbles.values())
                star      = " ⭐" if ver == active_ver else ""
                bubble_str = _("{} bubbles").format(n_bubbles) if n_bubbles != 1 else _("1 bubble")
                safe_print(_("    {}{:<6}  {:>4} pkgs   {}").format(ver, star, n_active, bubble_str))
                sys.stdout.flush()

            native_cfg_path = venv_root / "bin" / ".omnipkg_config.json"
            env_identity = _detect_env_identity(venv_root, _load_interp_config(native_cfg_path.parent))
            lock_data = {
                "meta": {
                    "omnipkg_version": _omnipkg_version(),
                    "native_python":   native_ver,
                    "env_name":        env_identity.get("env_name", ""),
                    "env_kind":        env_identity.get("env_kind", ""),
                },
                "python": python_data,
            }

            # now we have env identity — patch the header line
            env_name = env_identity.get("env_name", "")
            env_kind = env_identity.get("env_kind", "")
            omnipkg_ver = _omnipkg_version()
            # reprint header with full info (two blank lines to visually separate)
            safe_print("")
        except Exception as exc:
            safe_print(_("   ⚠️  Could not snapshot environment: {}").format(exc))

        # ── 6. Bubble sizes — stream each line as computed ────────────────────
        if self.multiversion_base.exists():
            safe_print(_("  Largest bubbles:"))
            sys.stdout.flush()

            pkg_groups: dict = defaultdict(list)
            total_bubble_mb = 0.0
            total_bubble_dirs = 0
            for bdir in self.multiversion_base.iterdir():
                if not bdir.is_dir() or bdir.name.startswith("."):
                    continue
                total_bubble_dirs += 1
                size_bytes = sum(f.stat().st_size for f in bdir.rglob("*") if f.is_file())
                total_bubble_mb += size_bytes / (1024 * 1024)
                parts   = bdir.name.rsplit("-", 1)
                pkg_key = parts[0].replace("_", "-") if len(parts) == 2 else bdir.name.replace("_", "-")
                pkg_groups[pkg_key].append((bdir.name, size_bytes))
            ...
            safe_print(_("  ({:.0f} GB total across {} dirs)").format(
                total_bubble_mb / 1024, total_bubble_dirs
            ))
            for pkg_name, dirs in sorted_pkgs[:10]:
                size_mb   = sum(s for _, s in dirs) / (1024 * 1024)
                n_ver     = len(dirs)
                ver_str   = _("{} version{}").format(n_ver, "s" if n_ver != 1 else "")
                prune_hint = _("  → 8pkg prune {}").format(pkg_name) if n_ver > 1 else ""
                safe_print(_("    {:<30}  {:>7.0f} MB   {}{}").format(
                    pkg_name, size_mb, ver_str, prune_hint
                ))
                sys.stdout.flush()

            remaining = total_bubble_dirs - sum(len(dirs) for _, dirs in sorted_pkgs[:10])
            if remaining > 0:
                safe_print(_("    ({} more dirs — run '8pkg prune' to clean up)").format(remaining))
