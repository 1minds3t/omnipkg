...
# Written at daemon startup with the installed omnipkg version.
# Checked on every run_cli — if the version changed the daemon exits so the
# next client invocation relaunches it with the freshly-installed code.
DAEMON_VERSION_STAMP_FILE = os.path.join(_VENV_TEMP_DIR, "omnipkg_daemon_version.txt")
...
class WorkerPoolDaemon:
    ...
    def _cleanup_stale_temp_files(self):
        ...
        _lock_base = os.path.join(tempfile.gettempdir(), "omnipkg")
        for _f in _glob.glob(os.path.join(_lock_base, "locks", "*.lock")) + \
                  _glob.glob(os.path.join(_lock_base, "bubble_tasks", "claimed_*.marker")):
            try: os.unlink(_f)
            except Exception: pass
        script_count = 0
        pip_dir_count = 0
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(_('Usage: python -m omnipkg.isolation.worker_daemon {start|stop|restart|status|logs}'))
        sys.exit(1)
