...
# In /home/minds3t/omnipkg/omnipkg/core.py

# [THE DEFINITIVE ENGINE - BASED ON THE "CLEAN ROOM" PRINCIPLE]
class BubbleIsolationManager:
    ...
    def create_isolated_bubble(self, package_name: str, target_version: str) -> bool:
        """Creates a bubble by performing a clean-room install into a temporary directory."""
        print(f"🫧 Creating isolated bubble for {package_name} v{target_version}")
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            
            # This is the single, definitive step to get the correct historical dependency tree.
            if not self._install_full_tree_isolated(package_name, target_version, temp_path):
                return False
            
            # Now that the temp directory is a perfect replica, we analyze and copy it.
            ...
            return self._copy_tree_with_deduplication(installed_tree, bubble_path, temp_path)

    def _install_full_tree_isolated(self, package_name: str, version: str, target_path: Path) -> bool:
        """
        Performs a full, clean installation into a temporary directory, ignoring the main environment.
        ...
        try:
            cmd = [
                ...
                "--ignore-installed",  # This is the magic flag. It forces a clean resolution from PyPI.
                f"{package_name}=={version}"
            ]
            print(f"    📦 Performing clean-room install for `{package_name}=={version}`...")
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode != 0:
                print(f"    ❌ Clean-room install failed: {result.stderr}")
                return False
            print("    ✅ Clean-room install successful.")
            return True
        except Exception as e:
            print(f"    ❌ Unexpected error during clean-room install: {e}")
            return False
    ...
    def _analyze_installed_tree(self, temp_path: Path) -> Dict[str, Dict]:
        ...
        for dist_info in temp_path.glob("*.dist-info"):
            try:
                ...
                pkg_files = [temp_path / f for f in dist.files or [] if (temp_path / f).exists()]
                installed[pkg_name] = {'version': dist.metadata['Version'], 'files': pkg_files, 'type': self._classify_package_type(pkg_files)}
            except Exception:
                continue
        return installed
    ...
    def _classify_package_type(self, files: List[Path]) -> str:
        ...
        return 'native' if has_native else 'pure_python'

    def _build_main_env_hash_index(self) -> Set[str]:
        print(f"    🔍 Building main environment file hash index for deduplication...")
        hash_set = set()
        ...
        print(f"    📈 Indexed {len(hash_set)} files.")
        return hash_set

    def _copy_tree_with_deduplication(self, installed_tree: Dict, bubble_path: Path, temp_install_path: Path) -> bool:
        print(f"    🧹 Creating final bubble with smart deduplication...")
        ...
        for pkg_name, pkg_info in installed_tree.items():
            for file_path in pkg_info['files']:
                total_files += 1
                should_copy = True
                # Deduplication logic: only skip if it's pure python and the hash matches
                if pkg_info['type'] == 'pure_python':
                    try:
                        if self._get_file_hash(file_path) in main_env_hashes:
                            should_copy = False
        ...
        print(f"    ✅ Bubble created: {copied_files} files copied, {deduplicated_files} deduplicated ({efficiency:.1f}% space saved).")
    ...
    def _create_bubble_manifest(self, bubble_path: Path, installed_tree: Dict):
        ...
        manifest = {'created_at': datetime.now().isoformat(), 'packages': {name: {'version': info['version']} for name, info in installed_tree.items()}, 'stats': {'bubble_size_mb': round(total_size / (1024 * 1024), 2), 'package_count': len(installed_tree)}}
        with open(bubble_path / '.omnipkg_manifest.json', 'w') as f: json.dump(manifest, f, indent=2)
...
class omnipkg:
    ...
    def smart_install(self, packages: List[str], dry_run: bool = False) -> int:
        ...
        # After all successful operations, save the snapshot.
    ...
    def _check_package_satisfaction(self, packages: List[str]) -> dict:
        """
        Checks if a package requirement is satisfied, prioritizing existing bubbles.
        This is the requested enhancement.
        """
        satisfied = set()
        needs_install = []
        ...
        for pkg_spec in packages:
            ...
            # --- START: Bubble Check Logic ---
            # Only perform this check for specific version requests (e.g., numpy==1.24.3)
            if '==' in pkg_spec:
                try:
                    pkg_name, version = pkg_spec.split('==', 1)
                    # Construct the expected path for the bubble directory
                    bubble_path = self.multiversion_base / f"{pkg_name}-{version}"
                    
                    # Check if the bubble directory exists
                    if bubble_path.exists() and bubble_path.is_dir():
                        satisfied.add(pkg_spec)
                        print(f"✅ Requirement already satisfied by existing bubble: {pkg_spec}")
                        is_satisfied = True
                except ValueError:
                    # Handle malformed specifiers, though pip usually catches these
                    pass
            # --- END: Bubble Check Logic ---
            
            if is_satisfied:
                continue

            # --- Fallback: Check Main Environment (Original Logic) ---
            try:
                # We need a package name even for non-versioned specs
                if pkg_name is None:
                    # Basic parsing for package name from specifier
                    pkg_name = re.split(r'[=<>~]', pkg_spec)[0]

                installed_version = importlib.metadata.version(pkg_name)
                
                # If a specific version was requested, check for an exact match
                if version and installed_version == version:
                    satisfied.add(pkg_spec)
                    print(f"✅ Requirement already satisfied in main environment: {pkg_spec}")
                    is_satisfied = True
                # A more general check could be added here for >=, <= if needed
                
            except importlib.metadata.PackageNotFoundError:
                # Package is not in the main environment, so it needs to be installed
                pass
