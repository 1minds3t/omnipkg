...
class omnipkgMetadataGatherer:
    ...
    def _store_in_redis(self, package_name: str, version_str: str, metadata: Dict):
        ...
        with self.redis_client.pipeline() as pipe:
            ...
            # Check if this version is in a bubble
            if version_path.is_dir():
                pipe.hset(main_key, f"bubble_version:{version_str}", "true")
                # Check bubble manifest for dependencies
                manifest_path = version_path / '.omnipkg_manifest.json'
                if manifest_path.exists():
                    with open(manifest_path, 'r') as f:
                        manifest = json.load(f)
                        for dep_name, dep_info in manifest.get('packages', {}).items():
                            dep_version = dep_info['version']
                            dep_key = f"{self.config['redis_key_prefix']}{dep_name.lower()}"
                            pipe.hset(dep_key, f"bubble_version:{dep_version}", "true")
                            # If this bubble is active, set dependency active versions
                            if os.environ.get(f"_omnipkg_ACTIVE_{pkg_name_lower.replace('-', '_').upper()}") == version_str:
                                pipe.hset(dep_key, "active_version", dep_version)
            else:
