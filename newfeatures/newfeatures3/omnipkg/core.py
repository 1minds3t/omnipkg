...
class omnipkg:
    ...
    def __init__(self, config_manager: ConfigManager):
        """
        Initializes the Omnipkg core engine with a given ConfigManager instance.
        """
        # Store the manager and the config data
        self.config_manager = config_manager
    ...
    def rebuild_knowledge_base(self, force: bool = False) -> int:
        ...
        try:
            ...
            cmd = [
                python_exe,
                "-m", "omnipkg.package_meta_builder",
            ...
            self._installed_packages_cache = None  # Fixed typo from original
    ...
    def _run_metadata_builder_for_delta(self, before: Dict, after: Dict):
        ...
        try:
            # --- THE FIX: Also pass the config path here ---
            python_exe = self.config["python_executable"]
            builder_script = self.config["builder_script_path"]
            config_file = self.config_manager.config_path
            ...
            result = subprocess.run(cmd, check=True, text=True, capture_output=True, timeout=600)
            if result.stdout:
                print(result.stdout)
            if result.stderr:
                print(result.stderr)
