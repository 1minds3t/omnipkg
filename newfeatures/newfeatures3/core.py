    ...
    def _load_or_create_config(self) -> Dict:
        ...
        with open(self.config_path, 'r') as f:
            try:
                ...
                return self._first_time_setup()
        defaults = self._get_sensible_defaults()
    print(f"DEBUG: Configured executable: {user_config.get('python_executable')}")
    print(f"DEBUG: Current executable: {defaults.get('python_executable')}")
        if user_config.get("python_executable") != defaults.get("python_executable"):
            print("🔄  Warning: A different Python environment was detected.")
            print(f"   Saved path: {user_config.get('python_executable')}")
            print(f"   Current path: {defaults.get('python_executable')}")
            
            choice = input("Do you want to update your config to use the current environment? (y/n): ")
            if choice.lower() == 'y':
