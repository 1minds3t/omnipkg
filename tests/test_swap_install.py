...
def test_rich_version():
    ...
    import sys
    import json
    print(_('--- Testing Rich in Python {} ---').format(sys.version[:5]), file=sys.stderr)
    print(_('--- Interpreter Path: {} ---').format(sys.executable), file=sys.stderr)
    try:
...
def run_command_with_streaming(cmd_args, description, python_exe=None):
    ...
    print(_('   Command: {}').format(' '.join(cmd)))
...
def get_current_env_id():
    ...
    try:
        ...
        print(_('⚠️  Could not get environment ID: {}').format(e))
        return None
...
def ensure_dimension_exists(version: str):
    ...
    print(_('   VALIDATING DIMENSION: Ensuring Python {} is adopted...').format(version))
    try:
        cmd = ['omnipkg', 'python', 'adopt', version]
        subprocess.run(cmd, check=True, capture_output=True, text=True)
        print(_('   ✅ VALIDATION COMPLETE: Python {} is available.').format(version))
    except subprocess.CalledProcessError as e:
        print(_('❌ FAILED TO ADOPT DIMENSION {}!').format(version), file=sys.stderr)
        print(_('--- Subprocess STDERR ---'), file=sys.stderr)
        print(e.stderr, file=sys.stderr)
        raise

def get_interpreter_path(version: str) -> str:
    """Asks omnipkg for the location of a specific Python dimension."""
    print(_('   LOCKING ON to Python {} dimension...').format(version))
    ...
    for line in result.stdout.splitlines():
        if line.strip().startswith(f'• Python {version}'):
            ...
            if match:
                ...
                print(_('   LOCK CONFIRMED: Target is at {}').format(path))
                return path
...
def prepare_dimension_with_rich(version: str, rich_version: str):
    ...
    print(_('   PREPARING DIMENSION {}: Installing rich=={}...').format(version, rich_version))
    python_exe = get_interpreter_path(version)
    print(_('🌀 TELEPORTING to Python {} dimension...').format(version))
    ...
    print(_('   ✅ TELEPORT COMPLETE. Active context is now Python {}.').format(version))
    print(f'   ⏱️  Dimension swap took: {swap_duration_ms:.2f} ms')
    env_id = get_current_env_id()
    if env_id:
        print(_('   📍 Operating in Environment ID: {}').format(env_id))
    ...
    try:
        if original_strategy != 'latest-active':
            print(_("   SETTING STRATEGY: Temporarily setting install_strategy to 'latest-active'..."))
        ...
        if current_strategy != original_strategy:
            print(_("   RESTORING STRATEGY: Setting install_strategy back to '{}'...").format(original_strategy))
    ...
    print(_('   ✅ PREPARATION COMPLETE: rich=={} is now available in Python {} context.').format(rich_version, version))
...
def rich_multiverse_test():
    ...
    if initial_env_id:
        print(_('📍 Initial Environment ID: {}').format(initial_env_id))
    test_results = []
    try:
        ...
        print(_('✅ All required dimensions are available.'))
        ...
        for py_version, rich_version in test_configs:
            ...
            print(_('   🧪 EXECUTING Rich test in Python {} dimension...').format(py_version))
            print(_('   📍 Using interpreter: {}').format(python_exe))
            start_time = time.perf_counter()
            try:
                cmd = [python_exe, __file__, '--test-rich']
                print(_('   🎯 Running command: {}').format(' '.join(cmd)))
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
                if result.returncode != 0:
                    print(f'   ❌ Rich test failed with return code {result.returncode}')
                    print(_('   STDOUT: {}').format(result.stdout))
                    print(_('   STDERR: {}').format(result.stderr))
                    continue
                if not result.stdout.strip():
                    print(_('   ❌ Rich test returned empty output'))
                    continue
            except subprocess.TimeoutExpired:
                print(_('   ❌ Rich test timed out after 30 seconds - SKIPPING'))
                continue
            except Exception as e:
            ...
            try:
                ...
                print(_('   ❌ Failed to parse JSON output: {}').format(result.stdout))
                continue
            test_data['execution_time_ms'] = (end_time - start_time) * 1000
            test_results.append(test_data)
            print(_('✅ Rich test complete in Python {}:').format(py_version))
            print(_('   - Rich Version: {}').format(test_data['rich_version']))
            print(_('   - Interpreter: {}').format(test_data['interpreter_path']))
            print(f"   ⏱️  Execution took: {test_data['execution_time_ms']:.2f} ms")
        print(_('\n🏆 MULTIVERSE RICH TEST COMPLETE!'))
        print(_('\n📊 RESULTS SUMMARY:'))
        print('=' * 80)
        for i, result in enumerate(test_results, 1):
            print(_('Test {}: Python {} | Rich {}').format(i, result['python_version'], result['rich_version']))
            print(_('   Interpreter: {}').format(result['interpreter_path']))
        ...
        if test_results:
            ...
            print(_('✅ Verified {} different Rich versions: {}').format(len(unique_versions), list(unique_versions)))
            print(_('✅ Verified {} different Python interpreters used').format(len(unique_interpreters)))
        ...
        print(_('STDOUT:'), file=sys.stderr)
        print(e.stdout, file=sys.stderr)
        print(_('STDERR:'), file=sys.stderr)
        print(e.stderr, file=sys.stderr)
        return False
    finally:
...
if __name__ == '__main__':
    if '--test-rich' in sys.argv:
        ...
        print('=' * 80)
        print(_('  🎨 RICH MULTIVERSE VERSION TEST'))
        print('=' * 80)
        ...
        print('=' * 80)
        if success:
            print(_('🎉🎉🎉 RICH MULTIVERSE TEST COMPLETE! Different Rich versions confirmed across Python interpreters! 🎉🎉🎉'))
        else:
