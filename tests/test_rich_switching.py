...
def pip_uninstall_rich():
    ...
    safe_print(_('   🧹 Using pip to uninstall rich from main environment...'))
    try:
        result = subprocess.run(['pip', 'uninstall', 'rich', '-y'], capture_output=True, text=True, check=False)
        if result.returncode == 0:
            safe_print(_('   ✅ pip uninstall rich completed successfully'))
        else:
            safe_print(_('   ℹ️  pip uninstall completed (rich may not have been installed)'))
        return True
    except Exception as e:
...
def pip_install_rich(version):
    ...
    safe_print(_('   📦 Using pip to install rich=={}...').format(version))
    try:
        result = subprocess.run(['pip', 'install', f'rich=={version}'], capture_output=True, text=True, check=True)
        safe_print(_('   ✅ pip install rich=={} completed successfully').format(version))
        return True
    except Exception as e:
...
def setup_environment():
    ...
    safe_print(_('   🧹 Cleaning up existing Rich installations and bubbles...'))
    ...
    if not pip_install_rich(LATEST_RICH_VERSION):
        safe_print(_('   ❌ Failed to install main environment Rich version'))
...
def create_test_bubbles(config_manager):
    ...
    for version in BUBBLE_VERSIONS_TO_TEST:
        safe_print(_('   🫧 Creating bubble for rich=={}').format(version))
        try:
            omnipkg_core.smart_install([f'rich=={version}'])
            safe_print(_('   ✅ Bubble created: rich-{}').format(version))
        except Exception as e:
            safe_print(_('   ❌ Failed to create bubble for rich=={}: {}').format(version, e))
    return BUBBLE_VERSIONS_TO_TEST

def test_python_import(expected_version: str, config_manager, is_bubble: bool):
    safe_print(_('   🔧 Testing import of version {}...').format(expected_version))
    ...
    try:
        ...
        if result.returncode == 0:
            safe_print(_('      └── {}').format(result.stdout.strip()))
            return True
        else:
        ...
        safe_print(f'   ❌ An unexpected error occurred while running the test subprocess: {e}')
        return False
    finally:
...
def run_comprehensive_test():
    ...
    try:
        ...
        for test_name, passed in test_results.items():
            ...
            safe_print(_('   {}: {}').format(test_name.ljust(25), status))
        if all_tests_passed:
            safe_print(_('\n🎉🎉🎉 ALL RICH LIBRARY TESTS PASSED! 🎉🎉🎉'))
            safe_print(_('🔥 OMNIPKG RICH HANDLING IS FULLY FUNCTIONAL! 🔥'))
        else:
        ...
        try:
            ...
            omnipkg_core = OmnipkgCore(config_manager)
            site_packages = Path(config_manager.config['site_packages_path'])
            safe_print(_('   🧹 Cleaning up test bubbles via omnipkg API...'))
            ...
            for cloaked in site_packages.glob('rich.*_test_harness_cloaked*'):
                safe_print(_('   🧹 Removing test harness residual cloaked: {}').format(cloaked.name))
                shutil.rmtree(cloaked, ignore_errors=True)
            safe_print(_('   📦 Restoring main environment: rich=={}').format(LATEST_RICH_VERSION))
            pip_uninstall_rich()
