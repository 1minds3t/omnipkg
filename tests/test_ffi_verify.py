...
def ffi_install(pkg):
    ...
    t = time.perf_counter()
    rc, unused, stderr, unused2 = run(cmd)

    ms = (time.perf_counter() - t) * 1000
    combined = "\n".join(str(x) for x in stderr) if isinstance(stderr, list) else stderr
    print(f"\n{'='*60}")
    print(f"CMD:    uv {cmd}")
...
safe_print(f"✅ PASS call 1: rich=={v1} on disk")
...
safe_print(f"✅ PASS call 2: rich=={v2} on disk")
...
safe_print(f"✅ PASS call 3: rich=={v3} on disk")

# Call 4 — already satisfied, should audit not reinstall
rc4, out4, v4 = ffi_install("rich==14.3.2")
assert rc4 == 0, f"FAIL: rc={rc4}"
# empty output is valid — uv silently no-ops when already satisfied
safe_print(f"✅ PASS call 4: no-op satisfied, rich=={v4} on disk")

print("\n" + "="*60)
safe_print("ALL 4 CALLS PASSED — FFI multi-call in-process verified ✅")
