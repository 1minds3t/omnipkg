...
def setup_environment():
    ...
    if not reset_omnipkg_environment():
        safe_print(_('   ⚠️  Reset failed, continuing anyway...'))
    omnipkg_core = OmnipkgCore(config_manager)
    ...
    for pkg in ['tensorflow', 'tensorflow_estimator', 'keras', 'typing_extensions']:
        for cloaked in site_packages.glob(f'{pkg}.*_omnipkg_cloaked*'):
            safe_print(_('   🧹 Removing residual cloaked: {}').format(cloaked.name))
            shutil.rmtree(cloaked, ignore_errors=True)
        for cloaked in site_packages.glob(f'{pkg}.*_test_harness_cloaked*'):
            safe_print(_('   🧹 Removing test harness residual cloaked: {}').format(cloaked.name))
            shutil.rmtree(cloaked, ignore_errors=True)
...
def run_script_with_loader(code: str, description: str):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if stderr_lines:
                safe_print(_('--- Relevant Errors ---'))
                for line in stderr_lines:
                    safe_print(line)
                safe_print('---------------------')
        return (result.returncode == 0, success_indicators)
    except subprocess.TimeoutExpired:
        safe_print(_('❌ Test timed out after 60 seconds'))
        return (False, [])
    except Exception as e:
        ...
        for line in info_result.stdout.splitlines():
            if '🎯 Active Context:' in line:
                ...
                if match:
                    ...
                    break
        safe_print(_('   ✅ Script running on Python {}.{}').format(sys.version_info.major, sys.version_info.minor))
        safe_print(_('   ✅ omnipkg active context: Python {}').format(active_version))
    except Exception as e:
        safe_print(_('   ⚠️  Could not determine omnipkg context: {}').format(e))
        safe_print(_('   ✅ Script running on Python {}.{}').format(sys.version_info.major, sys.version_info.minor))
    try:
        ...
        safe_print(_('Test 1 (TensorFlow 2.13.0 Bubble): {}').format('✅ PASSED' if success1 else '❌ FAILED'))
        safe_print(_('Test 2 (Dependency Switching): {}').format('✅ PASSED' if success2 else '❌ FAILED'))
        safe_print(_('Test 3 (Nested Loaders): {}').format('✅ PASSED' if success3 else '❌ FAILED'))
        safe_print(f'\\nOverall: {passed_tests}/{total_tests} tests passed')
        if passed_tests == total_tests:
            safe_print('🎉 ALL TESTS PASSED! omnipkgLoader is working correctly with TensorFlow!')
        elif passed_tests > 0:
            safe_print('⚠️  Some tests passed. The loader is partially functional.')
        else:
            safe_print('❌ All tests failed. There may be issues with bubble creation or the loader.')
